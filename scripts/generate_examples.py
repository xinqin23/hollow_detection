"""
Generate showcase figures for Overwatch character classification results.
...
DATA_DIR = PROJECT / "data" / "overwatch_original" / "test"
MODEL_PATH = PROJECT / "results" / "yolo_overwatch_runs" / "yolo_original" / "weights" / "best.pt"
RESULTS_DIR = PROJECT / "results"

# Alphabetically sorted class names (matching ImageFolder ordering)
CLASSES = [
    "Reaper", "Venture", "ashe", "bastion", "cassidy", "echo", "freja",
    "sojourn", "soldier", "sombra", "symmetra", "torbjourn", "tracer", "widowmaker",
]
...
def main():
    ...
    correct = []   # list of (img_path, true_label)
    incorrect = []  # list of (img_path, true_label, pred_label)
    ...
    # Pick diverse examples (spread across different classes)
    ...
    fig.suptitle("Correctly Classified Examples (YOLOv8n)", fontsize=18, fontweight="bold", y=0.98)
    ...
    out1 = RESULTS_DIR / "overwatch_correct_examples.png"
    ...
    fig, axes = plt.subplots(2, 5, figsize=(20, 9))
    fig.suptitle("Misclassified Examples (YOLOv8n)", fontsize=18, fontweight="bold", y=0.98)
    ...
    for j in range(len(incorrect_samples), 10):
        axes[j // 5, j % 5].axis("off")
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    out2 = RESULTS_DIR / "overwatch_incorrect_examples.png"
