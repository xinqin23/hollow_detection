"""
YOLOv8 classification on CIFAR-10 (original and hollow contour).
Uses ultralytics YOLOv8n-cls pretrained model, fine-tuned.
"""
import os
import sys
import json
...
def run_experiment(variant):
    """Run YOLO classification experiment on a dataset variant."""
    print(f"\n{'='*60}")
    print(f"YOLO Experiment: {variant}")
    print(f"{'='*60}")

    data_dir = DATA_ROOT / f"cifar10_{variant}"

    # Resume from last checkpoint if available, otherwise start fresh
    last_ckpt = RESULTS_DIR / "yolo_runs" / f"yolo_{variant}" / "weights" / "last.pt"
    if last_ckpt.exists():
        print(f"Resuming from {last_ckpt}")
        model = YOLO(str(last_ckpt))
        print(f"Training YOLOv8n-cls on {variant} data (resumed)...")
        t0 = time.time()
        results = model.train(resume=True)
        train_time = time.time() - t0
    else:
        model = YOLO("yolov8n-cls.pt")
        print(f"Training YOLOv8n-cls on {variant} data...")
        t0 = time.time()
        results = model.train(
            data=str(data_dir),
            epochs=NUM_EPOCHS,
    ...
    # Collect training history from YOLO results
    history = {"train_loss": [], "test_acc": []}
    csv_path = RESULTS_DIR / "yolo_runs" / f"yolo_{variant}" / "results.csv"
    if csv_path.exists():
        import pandas as pd
        df = pd.read_csv(csv_path)
        # YOLO CSV columns have spaces, strip them
    ...
    out_path = RESULTS_DIR / f"yolo_{variant}.json"
